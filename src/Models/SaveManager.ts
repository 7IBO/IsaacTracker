...
import { entitiesByIdAndVariant, entities } from "@/Helpers/Enums/Entities";
...
export class SaveManager {
  ...
  public async load(dataFile: Uint8Array): Promise<void> {
    ...
    this._version = Manipulation.getInt(
      this._data,
      Constants.VERSION_OFFSET,
      1,
    );
    let correct = Manipulation.is_correct_header(this._data, this._version);
    if (!correct) {
      throw new Error(
        "Invalid save file header! This save file is not compatible with the website.",
      );
    }

    this._sectionOffsets = Manipulation.getSectionOffsets(this._data);
    this._bestiaryOffsets = Manipulation.getBestiaryOffsets(
      this._data,
  ...
  private getSins(): number[] {
    let sinsOffset =
      this._sectionOffsets[2] - Constants.NUMBER_OF_ONLINE_SINS * 4 - 0x4 * 3;
  ...
  private setSins(index: number, value: number): void {
    let sinsOffset =
      this._sectionOffsets[2] -
      Constants.NUMBER_OF_ONLINE_SINS * 4 -
      0x4 * 3 +
      index * 4;
  ...
  private getStats(): Map<string, number> {
    ...
    let stats = new Map<string, number>(
      Constants.DICTIONNARY_STATISTICS_OFFSET,
    );
  ...
  public unlockSins(): void {
    let online =
      this._sectionOffsets[2] - Constants.NUMBER_OF_ONLINE_SINS * 4 - 0x4 * 5;
    this._data[online] = 1;
  ...
  private getChecklistUnlocks(charIndex: number): number[] {
    ...
    if (charIndex == 14) {
      ...
      // Characters before The Forgotten
  ...
  private getAchievements(): number[] {
    ...
    let achievements = new Array<number>(
  ...
  private getArrayBestiary(initialOffset: number): number[] {
    ...
    let bestiary = new Array<number>(entities.length).fill(0);
  ...
  private setArrayBestiary(array: number[], sectionNumber: number): number[] {
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      let variante = entities[i].variant;
  ...
  public setBestiary(
    ...
    Manipulation.setBestiaryOffsets(
      this._data,
      this._sectionOffsets,
      bestiaryLength,
    );
