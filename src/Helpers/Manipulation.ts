...
export class Manipulation {
  ...
  public static getBestiaryOffsets(
    data: Uint8Array,
  ...
  public static setBestiaryOffsets(
    data: Uint8Array,
    sectionOffsets: number[],
    length: number,
  ): void {
  ...
  public static getBestiaryLengths(
    data: Uint8Array,
    bestiaryOffset: number[],
  ): number[] {
    let res = [];
  ...
  public static calculateChecksum(
    ...
    for (let i = offset; i < offset + length; i++) {
      checksum =
        Constants.CRC_TABLE[(checksum & 0xff) ^ data[i]] ^ (checksum >>> 8);
    }
  ...
  public static is_correct_header(
    data: Uint8Array,
    ...
    for (let i = 0; i < Constants.HEADER_LENGTH; i++) {
      if (Manipulation.getInt(data, i, 1) != Constants.HEADER[i]) {
        console.error(
          `Header mismatch at index ${i}: expected ${
            Constants.HEADER[i]
          }, got ${Manipulation.getInt(data, i, 1)}`,
        );
        return false;
      }
    }
